...
public class CraftReflectionUtils {
	...
	public static org.bukkit.inventory.ItemStack ensureCraftItem(org.bukkit.inventory.ItemStack is) {
		...
		if (craftItemStack.isInstance(is))
			return is;

		try {
			return (org.bukkit.inventory.ItemStack) craftItemStack
					.getMethod("asCraftCopy", org.bukkit.inventory.ItemStack.class).invoke(null, is);
	...
	public static org.bukkit.inventory.ItemStack ensureNaturalBukkitItem(org.bukkit.inventory.ItemStack is) {
		if (is == null)
			return null;
		if (is.getClass() == org.bukkit.inventory.ItemStack.class)
			return is;

		try {
			Object nmsIS = getNMSItemStack(is);
			return (org.bukkit.inventory.ItemStack) craftItemStack.getMethod("asBukkitCopy", nmsItemStack).invoke(null,
					nmsIS);
		} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException | NoSuchMethodException
				| SecurityException e) {
			e.printStackTrace();
	...
	public static boolean setItemStackNMSTag(org.bukkit.inventory.ItemStack is, Object nmsTag) {
		if (nmsTag != null && !NBTCompoundReflection.isNBTCompound(nmsTag))
			return false;
		Object nmsIS = getNMSItemStack(is);
		if (nmsIS == null)
			return false;

...
public final class TagList extends Tag implements INBTList {
	...
	public byte[] getByteArray(int index) {
		try {
			return !this.hasKeyOfType(index, TagType.TAG_Byte_Array) ? new byte[0]
					: ((TagByteArray) this.list.get(index)).getByteArray();
		} catch (ClassCastException exception) {
			throw new RuntimeException("Type (" + Tag.getTagName(this.list.get(index).getType())
					+ ") different than expected (" + Tag.getTagName(TagType.TAG_Byte_Array) + ")", exception);
		}
	}
	...
	public int[] getIntArray(int index) {
		try {
			return !this.hasKeyOfType(index, TagType.TAG_Int_Array) ? new int[0]
					: ((TagIntArray) this.list.get(index)).getIntArray();
		} catch (ClassCastException exception) {
			throw new RuntimeException("Type (" + Tag.getTagName(this.list.get(index).getType())
					+ ") different than expected (" + Tag.getTagName(TagType.TAG_Int_Array) + ")", exception);
		}
	}
	...
	public long[] getLongArray(int index) {
		try {
			return !this.hasKeyOfType(index, TagType.TAG_Long_Array) ? new long[0]
					: ((TagLongArray) this.list.get(index)).getLongArray();
		} catch (ClassCastException exception) {
			throw new RuntimeException("Type (" + Tag.getTagName(this.list.get(index).getType())
					+ ") different than expected (" + Tag.getTagName(TagType.TAG_Long_Array) + ")", exception);
		}
	}
	...
	public TagCompound getCompound(int index) {
		try {
			return !this.hasKeyOfType(index, TagType.TAG_Compound) ? new TagCompound()
					: (TagCompound) this.list.get(index);
		} catch (ClassCastException exception) {
			throw new RuntimeException("Type (" + Tag.getTagName(this.list.get(index).getType())
					+ ") different than expected (" + Tag.getTagName(TagType.TAG_Compound) + ")", exception);
		}
	}
	...
	public TagList getList(int index, byte type) {
		try {
			...
			throw new RuntimeException("Type (" + Tag.getTagName(this.list.get(index).getType())
					+ ") diffenrent as expected (" + Tag.getTagName(TagType.TAG_List) + ")", exception);
		}
	}

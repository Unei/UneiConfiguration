...
public final class TagCompound extends Tag implements INBTCompound {
	...
	public boolean isUUID(String key) {
		return (this.hasKeyOfType(key + "Most", TagType.Number_TAG)
				&& this.hasKeyOfType(key + "Least", TagType.Number_TAG));
	}
	...
	public byte[] getByteArray(String key) {
		try {
			return !this.hasKeyOfType(key, TagType.TAG_Byte_Array) ? new byte[0]
					: ((TagByteArray) this.tags.get(key)).getByteArray();
		} catch (ClassCastException exception) {
			throw new RuntimeException("Type (" + Tag.getTagName(this.tags.get(key).getType())
	...
	public int[] getIntArray(String key) {
		try {
			return !this.hasKeyOfType(key, TagType.TAG_Int_Array) ? new int[0]
					: ((TagIntArray) this.tags.get(key)).getIntArray();
		} catch (ClassCastException exception) {
			throw new RuntimeException("Type (" + Tag.getTagName(this.tags.get(key).getType())
	...
	public long[] getLongArray(String key) {
		try {
			return !this.hasKeyOfType(key, TagType.TAG_Long_Array) ? new long[0]
					: ((TagLongArray) this.tags.get(key)).getLongArray();
		} catch (ClassCastException exception) {
			throw new RuntimeException("Type (" + Tag.getTagName(this.tags.get(key).getType())
